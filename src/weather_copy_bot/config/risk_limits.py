...
class RiskLimits:
    ...
    # Whitelists
    allowed_tokens: Optional[List[str]] = None  # None = all allowed
    ...
    def validate_drawdown(self, current_drawdown_pct: float) -> tuple[bool, str]:
        ...
        if current_drawdown_pct >= self.drawdown.warning_drawdown_pct:
            logger.warning(
                f"⚠️ Elevated drawdown: {current_drawdown_pct*100:.2f}%"
            )
...
class RiskValidator:
    ...
    def record_trade(self, pnl: float) -> None:
        ...
        if self._consecutive_losses >= self.limits.drawdown.max_consecutive_losing_days:
            logger.warning(
                f"⚠️ {self._consecutive_losses} consecutive losing days"
            )
