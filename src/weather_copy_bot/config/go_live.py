...
class PhaseProgress:
    ...
    def can_advance(self, criteria: PhaseCriteria) -> tuple[bool, List[str]]:
        ...
        if self.days_elapsed < criteria.min_duration_days:
            unmet.append(
                f"Duration: {self.days_elapsed}/{criteria.min_duration_days} days"
            )
        
        # Trade count check
        if self.trades_executed < criteria.min_trades:
            unmet.append(
                f"Trades: {self.trades_executed}/{criteria.min_trades}"
            )
...
class GoLiveManager:
    ...
    def __init__(
        ...
        logger.info(
            f"[GO-LIVE] Manager initialized with {len(phases)} phases"
        )
    ...
    def start_phase(self, phase: int, starting_balance: float) -> PhaseProgress:
        ...
        if phase != self.current_phase:
            raise ValueError(
                f"Cannot start phase {phase}, current is {self.current_phase}"
            )
        ...
        logger.info(
            f"[GO-LIVE] Started Phase {phase}: {self.phases[phase].name} "
            f"with ${starting_balance}"
        )
    ...
    def record_trade(
        ...
        if progress.peak_balance > 0:
            progress.current_drawdown_pct = (
                (progress.peak_balance - current_balance) / progress.peak_balance
            )
    ...
    def record_emergency_stop(self) -> None:
        ...
        if progress:
            ...
            logger.warning(
                f"[GO-LIVE] Emergency stop recorded: "
                f"{progress.emergency_stops} total"
            )
    ...
    def check_phase_completion(self) -> tuple[bool, List[str]]:
        ...
        if can_advance:
            logger.info(
                f"[GO-LIVE] Phase {phase} criteria met, can advance"
            )
        
        return can_advance, unmet

    def advance_phase(self, new_balance: float) -> Optional[PhaseProgress]:
        """
        ...
        if current_progress:
            ...
            current_progress.completed_at = datetime.now()
        ...
        logger.info(
            f"[GO-LIVE] Advanced to Phase {next_phase}: "
            f"{self.phases[next_phase].name}"
        )
        
        return next_progress

    def regress_phase(self) -> Optional[PhaseProgress]:
        """
        ...
        if prev_progress:
            ...
            prev_progress.started_at = datetime.now()
    ...
    def get_current_progress(self) -> Optional[PhaseProgress]:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_phase_name": (
                ...
                if self.current_phase in self.phases else None
            ),
            "total_phases": len(self.phases),
            "progress": {
                phase: p.to_dict()
                for phase, p in self.progress.items()
            },
    ...
    def generate_report(self) -> str:
        ...
        for phase_num in sorted(self.phases.keys()):
            ...
            if progress:
                if progress.status == PhaseStatus.ACTIVE:
                    ...
                    lines.append(
                        f"      Completed: {progress.completed_at.strftime('%Y-%m-%d')}"
                    )
        ...
        if current_config and current_progress:
            ...
            lines.append(f"  • Duration: {current_progress.days_elapsed}/{criteria.min_duration_days} days")
            ...
            win_rate = (
                ...
                if current_progress.trades_executed > 0 else 0
            )
...
def create_go_live_manager(
    ...
    """
    if conservative:
        # Use only first 2 phases
        phases = [p for p in DEPLOYMENT_PHASES if p.phase <= 2]
    else:
