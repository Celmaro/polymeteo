...
class OBIAnalyzer:
    ...
    def __init__(
        ...
        min_levels: int = 2,  # Min levels for OBI calculation
        max_spread_bps: float = 200.0,  # Max acceptable spread
        slope_threshold: float = 0.0,  # Book slope threshold
    ):
        """
    ...
    def calculate_obi(
        ...
        top_levels_imbalance = (
            (top_bid - top_ask) / (top_bid + top_ask) if (top_bid + top_ask) > 0 else 0.0
        )
        ...
        return OBIMetrics(
            ...
            vwm_spread=round(
                (asks[0][0] - bids[0][0]) / vwm_price * 10000 if vwm_price > 0 else 0, 2
            ),
