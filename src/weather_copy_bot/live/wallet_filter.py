...
WEATHER_KEYWORDS = {
    # Temperature
    "temperature", "temp", "heat", "cold", "warm", "freeze", "frost",
    "high temp", "low temp", "max temp", "min temp",
    # Precipitation
    "rain", "snow", "sleet", "hail", "precipitation", "precip",
    "rainfall", "snowfall", "showers", "storm",
    # Weather events
    "hurricane", "typhoon", "cyclone", "tornado", "blizzard",
    "fog", "wind", "gust", "thunder", "lightning",
    # General
    "weather", "forecast", "climate", "meteorological",
    "sunny", "cloudy", "overcast", "partly cloudy",
    # Winter specific
    "snowstorm", "ice", "freezing", "winter",
    # Summer specific  
...
RELATED_TOPICS = {
    "energy", "electricity", "heating", "cooling", "agriculture",
    "solar", "solar power", "wind power", "renewable",
}
...
class WeatherWalletFilter:
    ...
    def __init__(self, config: Optional[WalletFilterConfig] = None):
    ...
    def _find_related_topics(self, text: str) -> list[str]:
        ...
        found = []
        for topic in self.config.related_topics:
            if topic in text:
                found.append(topic)
        return found
...
class MultiWalletFilter:
    ...
    def add_wallet(
        self,
        wallet: str,
        config: Optional[WalletFilterConfig] = None,
    ) -> None:
    ...
    def get_all_stats(self) -> dict[str, Optional[FilterStats]]:
