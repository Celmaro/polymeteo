...
from .quorum import WalletTradeSignal, WalletCategory
...
class WeatherTagFilter:
    ...
    def __post_init__(self):
        if self.WEATHER_TAGS is None:
            self.WEATHER_TAGS = {
                # Temperature
                "temperature", "temp", "hot", "cold", "heat", "freeze", "frost",
                "warm", "cool", "celsius", "fahrenheit", "degree",
                
                # Precipitation
                "rain", "rainfall", "rainy", "snow", "snowfall", "sleet",
                "hail", "drizzle", "precipitation", "shower", "storm",
                
                # Weather events
                "hurricane", "typhoon", "cyclone", "tornado", "blizzard",
                "flood", "drought", "wildfire", "lightning", "thunder",
                "thunderstorm", "monsoon", "avalanche",
                
                # General weather
                "weather", "forecast", "climate", "atmospheric", "meteorological",
                "el_nino", "la_nina", "el niño", "la niña",
                
                # Cities and regions (for weather markets)
                "nyc", "new york", "los angeles", "chicago", "miami", 
                "houston", "phoenix", "seoul", "tokyo", "london", "paris",
                "berlin", "sydney", "singapore", "dubai",
...
class WeatherWalletFilter:
    ...
    # Common non-weather tokens to exclude
    EXCLUDED_PATTERNS = [
        r"^POLITICS", r"^POLI", r"^CRYPTO", r"^BTC", r"^ETH",
        r"^SPORTS", r"^ENTERTAINMENT", r"^STOCK",
    ]
    ...
    def classify_signal(
        ...
        if market_title:
            ...
            matched_tags = self.tag_filter.WEATHER_TAGS.intersection(
        ...
        if is_weather:
            ...
            logger.info(
                f"[Filter] ✅ Weather trade: score={score:.2f} ({', '.join(reasons)})"
            )
        else:
    ...
    def filter_signal(
        ...
        market_title: Optional[str] = None,
        token_id: Optional[str] = None,
    ) -> Optional[WalletTradeSignal]:
        """
        Filter a signal and return it if weather-related, None otherwise.
        """
        if self.is_wallet_tracked(signal.wallet_address):
            if self.classify_signal(signal, market_title, token_id):
                return signal
        return None
    ...
    def get_wallet_stats(self, address: str) -> Optional[dict]:
        ...
        return {
            ...
            "weather_ratio": (
                wallet.weather_trades_count / wallet.trades_count 
                if wallet.trades_count > 0 else 0
            ),
