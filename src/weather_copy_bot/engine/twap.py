...
class TWAPSlicer:
    ...
    def get_execution(self, execution_id: str) -> Optional[TWAPExecution]:
...
class TWAPIntegration:
    ...
    async def execute_with_twap(
        ...
        if size_usd <= self.threshold:
            logger.info(
                f"[TWAP-Int] Order size ${size_usd:.2f} below threshold, "
        ...
        if not risk_check.passed:
            logger.warning(
                f"[TWAP-Int] Risk check failed: {risk_check.reason}"
            )
            return None
