...
class LiquidityEstimator:
    ...
    def estimate_liquidity(
        ...
        for price, size in levels:
            if side.upper() == "BUY" and price <= target_price * (1 + self.max_slippage):
                depth_at_price += size
            elif side.upper() == "SELL" and price >= target_price * (1 - self.max_slippage):
                depth_at_price += size
...
class TWAPSlicerDepthAware:
    ...
    def __init__(
        ...
        # Depth-aware config
        liquidity_estimator: Optional[LiquidityEstimator] = None,
    ...
    def _get_order_book_provider(self) -> Optional[Callable]:
    ...
    def _calculate_adaptive_slices(
        ...
        base_num_slices = min(
            max(1, int(total_size_usd / self.min_slice_size)),
            self.max_slices
        )
        ...
        if initial_liquidity.max_slice_size > 0:
            adjusted_slices = min(
                max(1, int(total_size_usd / initial_liquidity.max_slice_size)),
                self.max_slices
            )
    ...
    async def execute(
        ...
        # Create slices
        slices = self._calculate_adaptive_slices(
            total_size_usd, initial_price, initial_liquidity
        )
    ...
    def get_execution(self, execution_id: str) -> Optional[DepthAwareExecution]:
