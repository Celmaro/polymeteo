...
class OrderQueue:
    ...
    def _is_duplicate(self, token_id: str, side: str) -> bool:
        ...
        self._dedup_index[key] = [
            oid for oid in self._dedup_index[key]
    ...
    async def enqueue(
        ...
        try:
            ...
            logger.error(f"[Queue] Queue full, order rejected")
            return None
    ...
    async def mark_filled(self, order_id: str, fill_amount: Optional[float] = None) -> bool:
    ...
    async def mark_rejected(self, order_id: str, reason: str = "") -> bool:
        ...
        logger.warning(f"[Queue] Order retry {order.retries}/{order.max_retries}: {order_id[:8]}...")
    ...
    def get_order_state(self, token_id: str, side: str) -> Optional[OrderState]:
    ...
    async def _process_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                queued_order = await asyncio.wait_for(
                    self._queue.get(),
                    timeout=1.0
                )
        ...
        while self._running:
            ...
            for order_id, order in list(self._active_orders.items()):
                if order.state in {OrderState.PENDING, OrderState.SUBMITTED}:
                    if now - order.created_at > stale_threshold:
