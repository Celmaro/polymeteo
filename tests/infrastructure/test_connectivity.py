...
class ConnectivityTester:
    ...
    def is_ready_for_live(self) -> tuple[bool, list[str]]:
        ...
        for result in self._results:
            ...
            elif result.latency_ms > self.thresholds.polymarket_api_fail and result.service == ServiceName.POLYMARKET_API.value:
                issues.append(f"{result.service}: latency too high ({result.latency_ms:.0f}ms)")

        # Polymarket API is critical
        polymarket = next(
            (r for r in self._results if r.service == ServiceName.POLYMARKET_API.value), None
        )
